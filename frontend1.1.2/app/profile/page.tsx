...
export default function ProfilePage() {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (userData.profileImageUrl && !userData.profileImageUrl.startsWith('http')) {
            userData.profileImageUrl = `${API_BASE_URL}${userData.profileImageUrl}`;
        }
        setUser(userData);
        ...
        reset({ ...otherUserData, friendCategories: categoriesForForm });
  ...
  const onUpdateProfile: SubmitHandler<FormInputs> = async (data) => {
    ...
    try {
      ...
      const response = await api.put<{ message: string, user: UserProfile }>('/users/profile', payload);
      const updatedUser = response.data.user;

      if (updatedUser.profileImageUrl && !updatedUser.profileImageUrl.startsWith('http')) {
          updatedUser.profileImageUrl = `${API_BASE_URL}${updatedUser.profileImageUrl}`;
      }
  ...
  const handleImageUpload = async () => {
    ...
    try {
      const response = await api.post<{ message: string, user: UserProfile }>('/users/profile/upload', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      const updatedUser = response.data.user;
    ...
    <div className="max-w-3xl mx-auto my-10 p-6 bg-white rounded-lg shadow-xl">
      ...
      <div className="flex flex-col items-center p-6 mb-8 bg-gray-50 rounded-lg">
        <Image
          ...
          width={150}
          height={150}
          className="w-80 h-100  object-cover border-10 border-gray-200 shadow-sm"
          priority
        />
        <div className="mt-4 flex items-center space-x-2">
          <label htmlFor="file-upload" className="cursor-pointer bg-gray-200 text-gray-700 px-4 py-2 rounded-md text-sm font-medium hover:bg-gray-300 transition-colors duration-200">
            Choose Image
          </label>
          <input id="file-upload" type="file" accept="image/*" onChange={handleFileChange} className="hidden" />
          {selectedFile && (
            <button onClick={handleImageUpload} disabled={isSubmitting} className="bg-indigo-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-indigo-700 disabled:bg-indigo-400 transition-colors duration-200">
      ...
      <form onSubmit={handleSubmit(onUpdateProfile)} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input id="firstName" {...register('firstName', { required: "First name is required" })} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
            {errors.firstName && <p className="text-red-500 text-xs mt-1">{errors.firstName.message}</p>}
          </div>
          <div>
            <label htmlFor="lastName" className="block text-sm font-medium text-gray-700">Last Name</label>
            <input id="lastName" {...register('lastName', { required: "Last name is required" })} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
            {errors.lastName && <p className="text-red-500 text-xs mt-1">{errors.lastName.message}</p>}
          </div>
        </div>
        <div>
          <label htmlFor="phone" className="block text-sm font-medium text-gray-700">Phone Number</label>
          <input id="phone" {...register('phone')} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
        </div>
        <div>
          <label htmlFor="address" className="block text-sm font-medium text-gray-700">Address</label>
          <textarea id="address" {...register('address')} rows={3} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
        </div>
        ...
        <div className="border-t pt-6">
          ...
          <div className="space-y-6 mt-4">
            <div>
              ...
              <textarea id="education" {...register('education')} rows={3} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
            </div>
            <div>
              <label htmlFor="experience" className="block text-sm font-medium text-gray-700">Experience</label>
              <textarea id="experience" {...register('experience')} rows={3} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
            </div>
            <div>
              <label htmlFor="skills" className="block text-sm font-medium text-gray-700">
                Skills <span className="text-gray-500 font-normal">(e.g., Cooking, Guitar, Programming)</span>
              </label>
              <input id="skills" {...register('skills')} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
            </div>
          </div>
        </div>
        ...
        {/* Friend Categories */}
        <div className="border-t pt-6">
          <h2 className="text-xl font-semibold text-gray-800">I am looking for a friend to...</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-4">
            {friendCategoryOptions.map(option => (
              <div key={option.id} className="flex items-center">
                <input
                  type="checkbox"
                  id={option.id}
                  {...register(`friendCategories.${option.id}`)}
        ...
        <div className="pt-5 text-right">
          <button
            ...
            className="w-full md:w-auto bg-blue-600 text-white px-6 py-2 rounded-md shadow-sm hover:bg-blue-700 disabled:bg-blue-300 disabled:cursor-not-allowed transition-all duration-200"
          >
